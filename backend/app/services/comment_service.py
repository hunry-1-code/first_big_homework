...
def persist_comment(article, raw: RawComment) -> Comment:
    ...
    preserve_llm = bool(
        row is not None
        and row.analysis_status == "llm"
        and row.content_hash == content_hash
    )
    if row is None:
        row = Comment(article_id=article.id, platform=raw.platform, source_comment_id=str(raw.source_comment_id), content_hash=content_hash)
        db.session.add(row)
    row.article_id = article.id; row.content = text; row.content_hash = content_hash; row.author = raw.author
