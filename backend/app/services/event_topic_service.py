...
CATEGORY_RULES = {
    ...
    "经济金融": ("经济", "股市", "银行", "基金", "房价", "汇率", "关税", "企业", "融资", "消费", "油价", "金价", "利率", "股票", "指数", "爆仓"),
    ...
    "教育": ("学校", "高考", "大学", "学生", "教师", "教育", "考试", "招生", "录取", "清华", "北大", "志愿", "专业"),
    "医疗健康": ("医院", "医生", "疾病", "药品", "医疗", "健康", "疫苗", "患者"),
    "体育": ("比赛", "冠军", "足球", "篮球", "奥运", "运动员", "联赛", "国足", "网球", "世界杯", "点球", "半决赛", "决赛", "女足"),
    "娱乐文化": ("电影", "电视剧", "明星", "演员", "歌手", "综艺", "演唱会", "票房", "音乐", "暑期档", "长剧", "短剧", "喜剧"),
...
def classify_topic_text(title: str, content: str = "") -> dict:
    text = f"{title or ''} {content or ''}"[:30000]
    scores = Counter(
        {
    ...
    return {
        "category": category,
        "topic_name": str(title or "")[:100],
        "confidence": confidence,
        "method": "event_rule_v1",
        "evidence": evidence,
    }
...
def classify_event_topic(event_id: int) -> dict:
    ...
    result = classify_topic_text(event.title or "", text)
    event.topic_category = result["category"]
    event.topic_name = result["topic_name"]
    db.session.commit()
    return result


__all__ = ["CATEGORY_RULES", "classify_event_topic", "classify_topic_text"]
