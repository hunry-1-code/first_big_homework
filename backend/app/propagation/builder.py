...
def build_propagation_graph(articles,platform_mapper=lambda x:x,max_nodes=40):
    ...
    return {'summary':{'node_count':len(nodes),'edge_count':len(links),'explicit_edge_count':sum(x['evidence_type']=='explicit' for x in links),'inferred_edge_count':sum(x['evidence_type']=='inferred' for x in links),'origin_candidate_count':len(roots),'platforms':sorted({n['platform'] for n in nodes}),'coverage_notice':f'基于已采集的{len(nodes)}篇报道重建传播网络，含{len(links)}条传播路径（{sum(x["evidence_type"]=="explicit" for x in links)}条显式、{sum(x["evidence_type"]=="inferred" for x in links)}条推断）'},'key_nodes':key_nodes,'phases':phases,'graph':{'nodes':nodes,'links':links,'secondary_links':[]}}
